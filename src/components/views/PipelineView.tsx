              ...
              <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-3 lg:gap-4 py-3 border-b border-slate-100 last:border-0">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-base lg:text-sm text-slate-900">{followup.name}</p>
                    <p className="text-sm lg:text-xs text-slate-500 truncate">Last: "{followup.lastMsg}"</p>
                  </div>
                ...
                <div className="flex gap-2 ml-12 sm:ml-0">
                  <button className="px-3 py-2 lg:py-1.5 rounded-full bg-slate-100 text-slate-600 text-sm lg:text-xs font-medium hover:bg-slate-200 transition-colors">
                    <Eye className="w-3.5 h-3.5 inline mr-1" />
                    Preview
                  </button>
                  <button className="px-3 py-2 lg:py-1.5 rounded-full bg-orange text-white text-sm lg:text-xs font-medium hover:bg-orange-500 transition-colors">
            ...
            {smartQueue.map((item, i) => (
              <div key={i} className="flex items-center gap-3 py-2 border-b border-slate-100 last:border-0">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-base lg:text-sm text-slate-900">{item.name}</p>
                  <p className="text-sm lg:text-xs text-slate-500">{item.detail}</p>
                </div>
            ...
            {scamAlerts.map((alert, i) => (
              <div key={i} className="p-4 rounded-xl bg-red-50/50 border border-red-100">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-base lg:text-sm text-slate-900 mb-1">{alert.title}</p>
                    <p className="text-sm lg:text-xs text-slate-600 leading-relaxed">{alert.desc}</p>
                    <div className="flex gap-2 mt-3">
                      <button className="px-3 py-2 lg:py-1.5 rounded-full bg-slate-100 text-slate-700 text-sm lg:text-xs font-medium hover:bg-slate-200 transition-colors">
                        View Details
                      </button>
                      <button className="px-3 py-2 lg:py-1.5 rounded-full text-sm lg:text-xs font-medium text-red-600 hover:bg-red-100 transition-colors">
                        Block and Report
                      </button>
                    </div>
                  </div>
